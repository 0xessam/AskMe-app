...
int ReadValidInt(int low, int high) {
...
int ShowReadMenu(vector<string> choices) {
    ...
    return ReadValidInt(1, choices.size());
}
...
struct Question {
    int question_id;
    int parent_question_id; // if the question isn't thread question = the parent id is -1
    int from_user_id;
    int to_user_id;
    ...
    Question(string line) {
        ...
        question_id = StringToint(substrs[0]);
        parent_question_id = StringToint(substrs[1]);
        from_user_id = StringToint(substrs[2]);
        to_user_id = StringToint(substrs[3]);
        is_anonymous_questions = StringToint(substrs[4]);
...
struct User {
    ...
    map<int, vector<int>> questionid_questionidsThead_to_map; // every main question have a list of thread questions ids (childrens)
    ...
    User(string line) {
        ...
        user_id = StringToint(substrs[0]);
        user_name = substrs[1];
        password = substrs[2];
        name = substrs[3];
        email = substrs[4];
        allow_anonymous_questions = StringToint(substrs[5]);
    }
...
struct QuestionsManager {
    ...
    void AnswerQuestion(User &user) {
        ...
        cout << "Enter answer(not contains commas \",\" ): ";
    ...
    void AskQuestion(User &user, pair<int, int> to_user_pair) {
        ...
        if (!to_user_pair.second) {
            ...
            cout << "Is anonymous questions?: (Enter 0 or 1): ";
...
struct UsersManager {
    ...
    void DoLogin() {
        LoadDatabase();	// update the database every time you login
